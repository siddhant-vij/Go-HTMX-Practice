...
func main() {
	...
	http.HandleFunc("/editGoal", func(w http.ResponseWriter, r *http.Request) {
		idx := r.FormValue("idx")
		if idx != "" {
			for i, goal := range courseGoals {
				if goal.Index == idx {
					editGoalForm := templates.EditFormDisplay(courseGoals[i].Index, courseGoals[i].Goal)
					editGoalForm.Render(r.Context(), w)
					break
				}
			}
		}
	...
	http.HandleFunc("/updateGoal", func(w http.ResponseWriter, r *http.Request) {
		idx := r.FormValue("idx")
		if idx != "" {
			for i, goal := range courseGoals {
				if goal.Index == idx {
					courseGoals[i].Goal = r.FormValue("goal")
					goalComponent := templates.DisplayGoal(courseGoals[i].Index, courseGoals[i].Goal)
					goalComponent.Render(r.Context(), w)
					break
				}
			}
	...
	http.HandleFunc("/cancelEdit", func(w http.ResponseWriter, r *http.Request) {
		idx := r.FormValue("idx")
		if idx != "" {
			for i, goal := range courseGoals {
				if goal.Index == idx {
